...
class Analysis_page(QtWidgets.QWidget):
    ...
    def getsamplesheet(self):
        ...
        file, _ = QFileDialog.getOpenFileName(self,"Choose Samplesheet", "","Sample sheet files (*.ss, *.samplesheet)", options=options)
        if file:
            self.samplesheet_lineEdit.setText(file)

    def getToolscfg(self):
        options = QFileDialog.Options()
        options |= QFileDialog.DontUseNativeDialog
        file, _ = QFileDialog.getOpenFileName(self,"Choose Tools configuration file", "","Tools configuration files (*.tools.cfg, *.tools.config)", options=options)
        if file:
    ...
    def getworkdir(self):
        ...
        folder = QFileDialog.getExistingDirectory(self,"Choose Working dir", os.path.expanduser("~") + '/Scrivania', options=options)
        if folder:
